			...
			switch(option){
				...
				case 2:
					...
					if(res==1){
						...
						view.printMessage(modelo.req1(ans[0], Double.parseDouble(ans[1]), Double.parseDouble(ans[2])));
					}
					else if(res==2){
						view.printMessage("Ingrese: minEnergia,maxEnergia,minDance,maxDance");
						String req = lector.next();
						String[] ans = req.split(",");
						ArregloDinamico<Reproduccion> este = modelo.req2(Double.parseDouble(ans[0]), 
																		 Double.parseDouble(ans[1]), 
																		 Double.parseDouble(ans[2]), 
																		 Double.parseDouble(ans[3]));
						view.imprimirVideoReq(este, este.size());
					}
					else if(res==4){
